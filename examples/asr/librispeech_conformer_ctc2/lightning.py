...
class ConformerCTCModule(LightningModule):
    def __init__(self, tokenizer, lexicon, config, inference_args=None):
        super().__init__()

        self.tokenizer = tokenizer
        self.lexicon = lexicon
        spm_vocab_size = self.tokenizer.get_piece_size()
        ...
        if self.inference_type == "greedy":
            tokens = {i: tokenizer.id_to_piece(i) for i in range(tokenizer.vocab_size())}
