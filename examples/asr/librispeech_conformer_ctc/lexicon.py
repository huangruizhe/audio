...
class Trie(object):
    ...
    def to_k2_str(self, node=None, start_index=0, last_index=-1, token2id=None):
        if node is None:
            node = self.root
        ...
        for i, (label, c) in enumerate(node.children.items()):
            if token2id is None:
                ...
                token = token2id[c.token]

            if len(c.children) > 0:
                res.append((start_index, f"{{x + {start_index}}} {{x + {next_index}}} {token} {token} {c.weight}"))
                _res, _next_index, _last_index = self.to_k2_str(node=c, start_index=next_index, last_index=last_index, token2id=token2id)
                next_index = _next_index
                res.extend(_res)
            else:
                res.append((start_index, f"{{x + {start_index}}} {{x + {last_index}}} {token} {token} {c.weight}"))
...
def test1():
    ...
    # trie.print()
    res, next_index, last_index = trie.to_k2_str(token2id=token2id)
    res = "\n".join(res)
    res = fstr(res, x=0)
